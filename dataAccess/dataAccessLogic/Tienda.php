    ...
    public function __construct(ConexionDB $connectionDB) {                                                                             
        ...
        public function setTotal(float $total): void {
            ...
            public function agregarProducto(): bool{                                                                                                                                   
                try {
                    $sql = "INSERT INTO tienda (producto, cantidad, precio, total) VALUES (?,?,?,?)";
...
public function eliminarCalculo(): bool{
    try {
        ...
        public function actualizarCalculo(): bool{
            try {
                        $sql = "UPDATE tienda SET producto = ?, cantidad = ?, precio = ?, total = ?  WHERE id = ?";
                        $stmt = $this->connectionDB->prepare($sql);
                        $stmt->execute([$this->getProducto(), $this->getCantidad(), $this->getPrecio(), $this->getTotal(), $this->getId()]);
                        $count = $stmt->rowCount();
                        return $this->affectedColumns($count);
                    } catch (PDOException $e) {
                        echo $e->getMessage();
